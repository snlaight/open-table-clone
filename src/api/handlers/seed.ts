...
const SeedDB = async (req: NextApiRequest, res : NextApiResponse<Data>) => {
  ...
  const vivaanId = restaurants.find((restaurant) => restaurant.name === 'Vivaan - fine Indian')
    ?.id || 1;
  const RamaKrishnaId = restaurants.find((restaurant) => restaurant.name === 'RamaKrishna Indian')
    ?.id || 1;
  const coconutLagoonId = restaurants.find((restaurant) => restaurant.name === 'Coconut Lagoon')
    ?.id || 1;
  const lastTrainToDelhiId = restaurants.find((restaurant) => restaurant.name === 'Last Train to Delhi')
    ?.id || 1;
  const adrakYorkvilleId = restaurants.find((restaurant) => restaurant.name === 'Adrak Yorkville')
    ?.id || 1;
  const curryishTavernId = restaurants.find((restaurant) => restaurant.name === 'Curryish Tavern')
    ?.id || 1;
  ...
  const kamasutraIndianId = restaurants.find((restaurant) => restaurant.name === 'Kamasutra Indian')
    ?.id || 1;
  const eldoradoTacoId = restaurants.find((restaurant) => restaurant.name === 'Eldorado Taco')?.id
    || 1;
  ...
  const mariachisId = restaurants.find((restaurant) => restaurant.name === '3 Mariachis')?.id
    || 1;
  const canoRestaurantId = restaurants.find((restaurant) => restaurant.name === 'Cano Restaurant')
    ?.id || 1;
  const bluRistoranteId = restaurants.find((restaurant) => restaurant.name === 'Blu Ristorante')
    ?.id || 1;
